/**
 * FILE: app/api/superAdmin/settings/vault-passphrase-generator/route.js
 ...
 *
 * PURPOSE:
 * GET  -> tells the generator page whether a vault passphrase has
 *         ever been set in the new VaultPassphrase table (never
 *         returns the passphrase or its hash).
 ...
 *         path (owner clicking this button, or a Gatekeeper 1/2
...
import { generateVaultPassphrase, hashVaultPassphrase, getVaultRecoveryUrl } from "@/services/vaultAuth";
 ...
 * generator page uses this to show "No passphrase set yet" vs "A
...
export async function GET(request) {
  ...
  try {
    ...
    console.error("[vaultPassphraseGenerator] Failed to check status:", error.message);
    return NextResponse.json(
...
export async function POST(request) {
  ...
  try {
    ...
    const reason = `Manually generated by ${adminProfile.fullName} from the admin dashboard (passphrase generator page)`;
    ...
    try {
      ...
      console.error("[vaultPassphraseGenerator] Failed to save passphrase to Google Drive:", driveError.message);
    }
    ...
    console.error("[vaultPassphraseGenerator] Failed to generate passphrase:", error.message);
    return NextResponse.json(
