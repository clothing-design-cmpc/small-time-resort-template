...
export async function POST(request) {
  ...
  let bannedDeviceId, totpCode;
  try {
    ({ bannedDeviceId, totpCode } = await request.json());
  } catch {
  ...
  try {
    ...
    // This is the highest-sensitivity route in the app (it reopens a
    // closed security door) — a crash here must still show up in the
    // Security Log rather than silently disappearing into a raw 500.
    await logSecurityEvent({
      eventType: "admin_login_denied",
      request,
      details: `Vault unban crashed unexpectedly: ${error.message}`,
    });
    console.error("[vault/unban] Unexpected error:", error.message);
    return NextResponse.json(
