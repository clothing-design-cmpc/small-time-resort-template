/**
 * FILE: app/gatekeeper-vault/[gatekeeperSlug]/GatekeeperVaultClient.jsx
 * ROLE: Gated by "gatekeeperVaultSession" only (checked server-side by
 *       the parent page before this ever renders) — no super_admin
 *       session cookie is involved.
 *
 * PURPOSE:
 * Lets whoever holds a valid Gatekeeper Vault session dry-run
 * Gatekeeper 1 (login brute force) and Gatekeeper 2 (booking SQL
 * injection) against THIS deployment, straight from the browser.
 * Wraps POST /api/gatekeeper-vault/run, which delegates the actual
 * work to services/gatekeeperTester.js. Same UI/behavior as the old
 * nav-linked Gatekeeper Tester page — only the location and the gate
 * in front of it have changed.
 *
 * DATA FLOW:
 ...
 * 3. Confirm -> POST /api/gatekeeper-vault/run
...
export default function GatekeeperVaultClient() {
  ...
  async function handleRunDryRun() {
    setIsRunning(true);
    try {
      const response = await axios.post("/api/gatekeeper-vault/run", { testIp1, testIp2 });
  ...
  return (
    <section className="gatekeeperVaultSection">
      <ToastStack toasts={toasts} onDismiss={dismissToast} />

      <div className="gatekeeperVaultHeaderRow">
        <span className="gatekeeperVaultEyebrow">Restricted Area</span>
        <h1 className="gatekeeperVaultTitle">Gatekeeper Tester</h1>
        <p className="gatekeeperVaultSubtitle">
      ...
      <div className="gatekeeperVaultWarningBanner">
      ...
      <div className="gatekeeperVaultCard">
        <div className="gatekeeperVaultFieldRow">
          <label className="gatekeeperVaultLabel" htmlFor="testIp1Input">
            Test IP 1 <span className="gatekeeperVaultFieldHint">(Gatekeeper 1 — login brute force)</span>
          </label>
          <input
            id="testIp1Input"
            className="gatekeeperVaultInput"
            type="text"
            value={testIp1}
        ...
        <div className="gatekeeperVaultFieldRow">
          <label className="gatekeeperVaultLabel" htmlFor="testIp2Input">
            Test IP 2 <span className="gatekeeperVaultFieldHint">(Gatekeeper 2 — booking SQL injection)</span>
          </label>
          <input
            id="testIp2Input"
            className="gatekeeperVaultInput"
        ...
        <p className="gatekeeperVaultFieldNote">
        ...
        <button
          type="button"
          className="gatekeeperVaultRunButton"
      ...
      {result && (
        <div className="gatekeeperVaultResultsCard">
          <div className="gatekeeperVaultResultsHeader">
            <h2 className="gatekeeperVaultResultsTitle">Results</h2>
            <span
              className={
                result.allPassed ? "gatekeeperVaultResultsSummaryPass" : "gatekeeperVaultResultsSummaryFail"
              }
            >
          ...
          <ul className="gatekeeperVaultChecklist">
            {result.checks.map((check) => (
              <li key={check.name} className="gatekeeperVaultChecklistItem">
                <span className={check.passed ? "gatekeeperVaultPillPass" : "gatekeeperVaultPillFail"}>
                  {check.passed ? "Pass" : "Fail"}
                </span>
                <span className="gatekeeperVaultChecklistText">
                  {check.name}
                  {check.detail && <span className="gatekeeperVaultChecklistDetail"> — {check.detail}</span>}
                </span>
              </li>
            ))}
          </ul>
      ...
      <p className="gatekeeperVaultDocsNote">
