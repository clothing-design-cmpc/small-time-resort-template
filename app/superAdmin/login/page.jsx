...
export default function SuperAdminLoginPage() {
  ...
  async function onSubmit(data) {
    ...
    if (!result.success) {
      ...
      // correct-password login. The route has already set a
      // "loginOtpChallenge" cookie; hand off to its own page (same
      // pattern the vault uses for its passphrase -> OTP handoff)
      // instead of rendering the challenge inline here.
      if (result.data?.otpRequired) {
        router.push("/superAdmin/login/otp");
        return;
      }
