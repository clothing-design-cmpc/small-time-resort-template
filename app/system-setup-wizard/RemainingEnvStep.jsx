...
<body>
  ...
  vaultSecurity: {
    ...
    steps: [
      ...
      "CRON_SECRET authenticates the nightly automated call to /api/system-vault-setup/auto-rotate — this is only exercised once the site is deployed and the scheduled cron job is wired up (see the Deployment step later in this wizard). Nothing to test locally for this one right now — just generate the value and save it.",
      ...
      "The vault passphrase itself (the value that pairs with VAULT_SETUP_KEY above) is set later, in Step 6 of this wizard, using node scripts/setupVaultPassphrase.js — a separate terminal-only script, not something generated here.",
    ],
    codeBlocksIntro:
      "The command below goes into a terminal — Command Prompt, PowerShell, or Git Bash on Windows; Terminal on macOS/Linux — opened at your project's root folder (the same folder that contains package.json).",
    codeBlocks: [
      {
  ...
  aiInsightAndDirections: {
    ...
    steps: [
      "Google Cloud, Step 0 — create your project (skip this whole step only if you already have a Google Cloud project from before): go to console.cloud.google.com in your browser. If it asks you to sign in, use your Gmail account — any Gmail works, it doesn't need to be special.",
      "Google Cloud, Step 0 (continued) — look at the very top of the page. Next to the small \"Google Cloud\" logo, there is a button. It might say \"Select a project\" in gray text, or it might already show a project name from something else. Click that button.",
      "Google Cloud, Step 0 (continued) — a small window pops up in the middle of the screen. Look at the top-right corner of that popup window and click the blue button that says \"NEW PROJECT\".",
      "Google Cloud, Step 0 (continued) — a new page appears asking for a \"Project name\". Click inside that box and type a simple name so you remember it later, for example: villa-azure-resort (all lowercase, you can use dashes instead of spaces). Leave every other field on this page exactly as it already is — don't change \"Location\" or anything else, those are automatic and normal to leave alone.",
      "Google Cloud, Step 0 (continued) — click the blue \"CREATE\" button at the bottom of that page. Now just wait — a small bell icon (notifications) near the top-right of the screen will spin for about 10 to 30 seconds while Google sets up your new project. Don't click anything else while it's spinning.",
      "Google Cloud, Step 0 (continued) — once the bell icon stops spinning, click the same project dropdown at the top again (same spot as before, next to the \"Google Cloud\" logo). This time you'll see the project name you just typed in a list — click on it to select it. The page will refresh and that project name will now stay visible at the top of every Google Cloud page from now on. This confirms it worked — you now have a Google Cloud project ready to use for both the Gemini step below and the Maps + Weather steps further down.",
      "Gemini: in AI Studio, click \"Get API key\" then \"Create API key\". If you just made a new project in Step 0 above, select that same project when it asks which one to use (it may say \"Import an existing Google Cloud project\" — pick the one you just created). No billing account required for Gemini. Copy the key into GEMINI_API_KEY.",
