...
const SETUP_GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Maps + Weather, step E — enable each API one at a time: in the Library's search box, type the exact name below, click the matching result card, then click the blue \"Enable\" button on that page. If a billing prompt appears at this point (something like \"This API requires a billing account\", with a dropdown) — that's normal the first time any API is enabled on a brand-new project, even if you already have a billing account from an earlier project. Pick your existing billing account from that dropdown and click \"Set account\" / \"Confirm\" — this just links THIS project to your billing account (a one-time, per-project step), it does not create a new charge. Then wait for the API to finish enabling (a few seconds), click the browser's back button (or search again) to enable the next one. Repeat for all FOUR names: \"Geocoding API\", \"Maps Static API\", \"Routes API\", \"Weather API\". Maps Static API is the easiest one to forget since nothing else on this page reminds you about it — skipping it won't show any error anywhere, it just means the little map picture on /visitor/directions never shows up. Do this step BEFORE step F below — creating the key first, with nothing enabled yet, causes a \"No APIs selected\" / \"API selection required\" error when you try to save it.",
