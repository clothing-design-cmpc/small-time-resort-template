...
const SETUP_GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Google Maps + Weather: go to console.cloud.google.com, use the same project (or a new one). Unlike Gemini, these APIs require billing enabled — left menu (☰) → Billing → \"Link a billing account\" → add a payment card first. This unlocks enabling the APIs below; it doesn't charge you by itself — all four have a free monthly call allowance well above what this project needs.",
      "APIs & Services → Library — search for and enable each of these FOUR APIs one at a time: Geocoding API, Maps Static API, Routes API, Weather API. Maps Static API is easy to miss since nothing in the app calls it by that exact name in visible UI text — skipping it doesn't error loudly, it just makes the route map image silently never appear on /visitor/directions.",
      "APIs & Services → Credentials → Create Credentials → API Key. It's created unrestricted by default — you won't be charged unless you exceed the free allowance, but restrict it anyway (next step).",
      "Restrict the key to all four APIs from the step above (API restrictions, not \"Don't restrict key\") — Geocoding, Maps Static, Routes, and Weather.",
