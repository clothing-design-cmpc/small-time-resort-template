 ...
 * 2. Country/city is resolved from the request IP in-memory via
 *    geoip-lite (same local, no-network-call approach as services/analytics.js)
...
function resolveLocation(request) {
  ...
  try {
    ...
    return {
      ipAddress,
      geoCity: lookup?.city ?? null,
      geoCountry: lookup?.country ?? null,
    };
  } catch {
...
export async function recordAccountActivity({ request, accountId, action }) {
  ...
  try {
    const { ipAddress, geoCity, geoCountry } = resolveLocation(request);
