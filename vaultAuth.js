 ...
 * hash alone: sha256(effectiveHash), first 7 hex characters.
 * Deliberately hashes the hash again rather than slicing the stored
 * "salt:hash" value directly — the stored value is itself a
 * credential, and a public URL segment is the last place a fragment
 * of it should ever appear, even indirectly. Returns null if no
 * passphrase hash is configured yet at all (fresh install, before the
 * one-time hashVaultPassphrase.js setup step) — callers must treat
 * null as "the vault isn't configured, nothing should resolve".
 */
